...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Check if store with this name already exists for this character
    const existing = await ctx.db
      .query("dataStores")
      .withIndex("by_character_name", (q) => 
        q.eq("characterId", args.characterId).eq("name", args.name)
      )
      .first();
...
// List all stores for a character
...
export const getEntriesByStoreName = query({
  ...
  handler: async (ctx, args) => {
    const store = await ctx.db
      .query("dataStores")
      .withIndex("by_character_name", (q) => 
        q.eq("characterId", args.characterId).eq("name", args.storeName)
      )
      .first();
